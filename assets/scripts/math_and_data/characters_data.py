...
def marisa_base_attack(fire_point: Vector2, power: int):
    ...
    bullet_data = BulletData(characters[0]["bullet-sprite-sheet"], Collider(scale_distance(5), offset=Vector2.up() * scale_distance(10)), sprite_scale=WORLD_SCALE)

    for i in range(-current_power, current_power + 1):
        i /= 2
        bullet_1 = PlayerBullet(bullet_data, fire_point - Vector2(scale_distance(6), 0), delta_angle * i, scale_distance(900), damage=1)
        bullet_2 = PlayerBullet(bullet_data, fire_point + Vector2(scale_distance(6), 0), delta_angle * i, scale_distance(900), damage=1)
